...
export class CairoContractWriter extends CairoASTNodeWriter {
  writeInner(node: CairoContract, writer: ASTWriter): SrcDesc {
    ...
    const storageCode = [
      '@storage_var',
      'fn WARP_STORAGE(index: felt) -> (val: felt){',
      '}',
      '@storage_var',
      'fn WARP_USED_STORAGE() -> (val: felt){',
      '}',
      '@storage_var',
      'fn WARP_NAMEGEN() -> (name: felt){',
      '}',
      'fn readId(loc: felt) -> (val: felt) implicits(syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, RangeCheck){',
      ...
      ...(INCLUDE_CAIRO_DUMP_FUNCTIONS
        ? [
            'fn DUMP_WARP_STORAGE_ITER(length : felt, ptr: felt*) implicits(syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, RangeCheck){',
            '    if (length == 0){',
            '        return ();',
            '    }',
            '    let index = length - 1;',
            ...
            '}',
            '#[external]',
            'fn DUMP_WARP_STORAGE(length : felt) -> (data_len : felt, data: felt*) implicits(syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, RangeCheck){',
            '    let (p: felt*) = alloc();',
